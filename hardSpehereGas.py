...
animation = vp.canvas(width=win, height=win, align='left')
animation.range = L
...
boxbottom.append([vp.vector(-d, -d, -d), vp.vector(-d, -d, d),
                  vp.vector(d, -d, d), vp.vector(d, -d, -d), vp.vector(-d, -d, -d)])
boxtop = vp.curve(color=gray, radius=r)
boxtop.append([vp.vector(-d, d, -d), vp.vector(-d, d, d),
               vp.vector(d, d, d), vp.vector(d, d, -d), vp.vector(-d, d, -d)])
...
p = []
apos = []
pavg = vp.sqrt(2*mass*1.5*k*T)  # average kinetic energy p**2/(2mass) = (3/2)kT
...
for i in range(Natoms):
    ...
    z = L*vp.random()-L/2
    if i == 0:
        Atoms.append(vp.sphere(pos=vp.vector(x, y, z), radius=Ratom, color=vp.color.cyan,
                               make_trail=True, retain=100, trail_radius=0.3*Ratom))
    else:
        Atoms.append(vp.sphere(pos=vp.vector(
            x, y, z), radius=Ratom, color=gray))
    apos.append(vp.vector(x, y, z))
    theta = vp.pi*vp.random()
    phi = 2*vp.pi*vp.random()
    px = pavg*vp.sin(theta)*vp.cos(phi)
    py = pavg*vp.sin(theta)*vp.sin(phi)
    pz = pavg*vp.cos(theta)
    p.append(vp.vector(px, py, pz))
...
gg = vp.graph(width=win, height=0.4*win, xmax=3000, align='left',
...
for v in range(0, 3001+dv, dv):  # theoretical prediction
    theory.plot(v, (deltav/dv)*Natoms*4*vp.pi*((mass/(2*vp.pi*k*T))**1.5)
